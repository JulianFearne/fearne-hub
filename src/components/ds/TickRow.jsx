export default function TickRow({ checked, onToggle, qty, trail, disabled, className = '', children }) {
  return (
    <div className={`fh-tick-row${trail ? ' fh-tick-row--trail' : ''} ${className}`.trim()}>
      <button type="button" className="fh-tick" role="checkbox" aria-checked={checked} onClick={onToggle} disabled={disabled}>
